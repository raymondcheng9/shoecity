        ...
        elseif (Tools::isSubmit('partialRefund') && isset($order)) {
            if ($this->tabAccess['edit'] == '1') {
                if (Tools::isSubmit('partialRefundProduct') && ($refunds = Tools::getValue('partialRefundProduct')) && is_array($refunds)) {
                    ...
                    {
                        $transcation = $payment->transaction_id;
                        $paymentMethod = $payment->payment_method;
                        if($transcation&&$amount<=$payment->amount)
                        {

                            include_once(_PS_ROOT_DIR_.'/modules/stripem/stripe-php/init.php');

                            $stripe_key = '';
                            if(Configuration::get('STRIPE_SANDBOX')=='1')
                            {
                                $stripe_key =Configuration::get('STRIPE_TEST_KEY');
                            }else{
                                $stripe_key =Configuration::get('STRIPE_LIVE_KEY');
                            }
                            \Stripe\Stripe::setApiKey($stripe_key);
                            //\Stripe::setApiKey($stripe_key);

                            try{
                                $refund = \Stripe\Refund::create(array('amount' => $amount*100, 'charge' => $transcation));

                            }catch (Exception $e)
                            {
                                $this->errors[] = Tools::displayError('cant refund from Stripe.');
